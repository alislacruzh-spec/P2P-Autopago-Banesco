...
function buscarMontoEstructural(raiz) {
    ...
    if (location.host.includes("binance.com")) {
        ...
        // ★ El botón "Transferencia telefónica" solo se crea del lado de
        // Banesco (ver más abajo) — en Binance no hace falta mostrarlo, ya
        // que el estado se lee de GM storage igual, sin importar desde
        // dónde se haya activado.
    ...
    if (location.host.includes("banesconline.com")) {
        ...
        //
        // ★ NOTA: #bancocta y #NacCliCta ya NO se pegan acá. Salieron a
        // intentarAplicarBancoCta()/intentarAplicarNacCta() (más abajo),
        // que los aplica como campos "estructurales" UNA sola vez, ANTES de
        // que arranque este ciclo de reintentos. Motivo: cambiar el <select>
        // de Banco dispara un postback real del lado de Banesco (igual que
        // TipTrans) — si se pegaba junto con cuenta/monto/cédula en la misma
        // pasada, la recarga que eso provocaba borraba los datos
        // transaccionales que ya se habían pegado un instante antes.
        ...
        // PASO 3b — Banco (#bancocta), SOLO para "Código de Cuenta"
        // (cuenta_tercero_opc30). Se aplica una única vez, DESPUÉS de
        // confirmar TipTrans, exactamente con el mismo criterio que TipTrans
        // mismo: seleccionar un banco en este <select> dispara un postback
        // real del lado de Banesco. Si esto se pegaba junto con cuenta/monto/
        // cédula en la misma pasada (como pasaba antes), la recarga que
        // provocaba borraba los datos transaccionales recién pegados —
        // exactamente el bug de "pega bien y al toque se recarga y se borra".
        let bancoCtaAplicado = false;

        function intentarAplicarBancoCta(bancoCodigo) {
            if (bancoCtaAplicado) return true;
            if (!bancoCodigo) { bancoCtaAplicado = true; return true; } // nada que aplicar
            const elBanco = document.querySelector("#bancocta");
            if (!elBanco) return false; // el <select> aún no existe en el DOM
            const ok = setSelectByValue(elBanco, bancoCodigo + '|A');
            if (ok) {
                bancoCtaAplicado = true;
                log('Auto Select: banco (bancocta) confirmado — se aplica antes de pegar cuenta/monto para evitar el postback que borraba los datos.');
            }
            return ok;
        }

        // PASO 3c — Nacionalidad (#NacCliCta), misma razón que el banco:
        // se aplica una única vez, ANTES del ciclo transaccional, por si
        // también dispara algún postback (no confirmado, pero mejor
        // prevenir dado que es el mismo tipo de <select> AutoPostBack).
        let nacCtaEvaluado = false;

        function intentarAplicarNacCta(cedulaRaw) {
            if (nacCtaEvaluado) return true;
            if (!cedulaRaw) { nacCtaEvaluado = true; return true; } // nada que evaluar
            aplicarNacionalidadSegunCedula(document.querySelector("#NacCliCta"), cedulaRaw, 'NacCliCta');
            nacCtaEvaluado = true;
            return true;
        }
        ...
        // la estructura completa ya quedó lista: cuenta de origen + TipTrans
        // siempre, y además banco + nacionalidad cuando el tipo es
        // "Código de Cuenta" (cuenta_tercero_opc30).
        function intentarAplicarEstructura() {
            ...
            if (!tipTransOk) return false;

            if (payload && payload.tipo === 'cuenta_tercero_opc30') {
                const bancoOk = intentarAplicarBancoCta(payload.bancoCodigo);
                const nacOk   = intentarAplicarNacCta(payload.cedula);
                return bancoOk && nacOk;
            }
